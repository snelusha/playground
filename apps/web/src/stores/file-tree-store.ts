...
import type { PlaygroundAsyncFS } from "@/lib/fs/playground-async-fs";
...
type FileTreeActions = {
	init(fs: PlaygroundAsyncFS): void;
...
export const useFileTreeStore = create<FileTreeState & FileTreeActions>()(
	immer((set, get) => {
		let fs: PlaygroundAsyncFS | null = null;

		const _fs = (): PlaygroundAsyncFS => {
		...
		return {
			...
			init(instance) {
				...
				fs = instance;
				set((s) => {
					s.remoteConnected = instance.remoteEnabled();
				});
				void (async () => {
			...
			async _syncTrees() {
				const pfs = _fs();
				const remote = await pfs.remoteTree();
				set((s) => {
					s.tempTree = pfs.tempTree();
					s.localTree = pfs.localTree();
					s.remoteTree = remote;
				});
			},

			async loadSharedFiles(root, openRelativePath) {
...
export const useRemoteTree = () => useFileTreeStore((s) => s.remoteTree);
export const useRemoteConnected = () =>
	useFileTreeStore((s) => s.remoteConnected);
