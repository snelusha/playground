...
func wasmPal(stderr, stdout io.Writer, fsys *bridgeFS, workingDir string) pal.Platform {
	...
	return pal.Platform{
		...
		FS: pal.FS{
			ReadFile: func(p string) ([]byte, error) {
				return fs.ReadFile(fsys, resolvePath(p))
			},
			WriteFile: func(p string, data []byte) error {
				return fsys.WriteFile(resolvePath(p), data, 0o644)
			},
